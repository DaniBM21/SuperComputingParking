...

{
    ...
    echo -e "\n\n############Información CPU############"
    echo "Arquitectura: $(lscpu | grep "Architecture:" | awk '{print $2}')"
    echo "CPU(s): $(lscpu | grep -m1 "CPU(s):" | awk '{print $2}')"
    echo "Frecuencia: $(lscpu | grep "Model name:" | awk '{print $(NF)}' ORS=' ')"
    ...
    echo "Load Average (1 min): $(uptime | awk '{print $8}' | cut -d, -f1,2)"
    echo "Load Average (5 min): $(uptime | awk '{print $9}' | cut -d, -f1,2)"
    echo "Load Average (15 min): $(uptime | awk '{print $10}' | cut -d, -f1,2)"
